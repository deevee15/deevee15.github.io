...
function play(id){
    ...
    var percentageOfVolumeMeter = document.getElementById('audio_stats_vslider').offsetWidth * percentageOfVolume;
    
    //Fills out the volume status bar.
    document.getElementById('audio_stats_volume').style.width = Math.round(percentageOfVolumeSlider) + "px";
}
...
function playPause(id){
    ...
    if (activeSong.paused){
         activeSong.play();
         $('#audio_play').css({"background":"#42aaff url(/img/player.png) 0px -56px"});
    }else{
         activeSong.pause();
         $('#audio_play').css({"background":"#42aaff url(/img/player.png) 0px 0px"});
    }
}
...
function updateTime(){
    ...
    document.getElementById('audio_stats_time').innerHTML = currentMinutes + ":" + currentSeconds + ' / ' + Math.floor(activeSong.duration / 60) + ":" + (Math.floor(activeSong.duration % 60) < 10 ? '0' : '') + Math.floor(activeSong.duration % 60);

    //Fills out the slider with the appropriate position.
    var percentageOfSong = (activeSong.currentTime/activeSong.duration);
    var percentageOfSlider = document.getElementById('audio_stats_tslider').offsetWidth * percentageOfSong;
    
    //Updates the track progress div.
    document.getElementById('audio_stats_track').style.width = Math.round(percentageOfSlider) + "px";
}
